...
import (
	...
	"github.com/sivchari/chat-example/pkg/domain/repository/message/mock_message"
	"github.com/sivchari/chat-example/pkg/domain/repository/room/mock_room"
	"github.com/sivchari/chat-example/pkg/errors"
	"github.com/sivchari/chat-example/pkg/ulid/mock_ulid"
...
func TestIntercator_CreateRoom(t *testing.T) {
...
func TestInteractor_GetRoom(t *testing.T) {
	t.Run("【正常系】", func(t *testing.T) {
		...
		name := "name"
		ctx, i, m := newWithMocks(t)
		m.roomRepository.EXPECT().Select(ctx, id).Return(&entity.Room{
			ID:   id,
			Name: name,
		}, nil).Times(1)

		res, err := i.GetRoom(ctx, id)
		assert.Equal(t, &entity.Room{
			ID:   id,
			Name: name,
		}, res)
	...
	t.Run("【異常系】idが空文字列", func(t *testing.T) {
		id := ""
		ctx, i, _ := newWithMocks(t)
		res, err := i.GetRoom(ctx, id)
		assert.Nil(t, res)
